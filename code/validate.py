...
df_new = pd.read_csv(os.path.join(os.environ['DATA_PATH'])) 
...
visualize_seed_point(df_new.iloc[0])

# def validate(row):
#     img = sitk.ReadImage(row.image_path)
#     x, y, z = img.GetSize()
#     if x < row.coordX or y < row.coordY or z < row.coordZ:
#         return x, row.coordX, y, row.coordY, z, row.coordZ

# print(Parallel(n_jobs=-1)(delayed(validate)(row) for row in tqdm(df_new.itertuples(), total=len(df_new))))
